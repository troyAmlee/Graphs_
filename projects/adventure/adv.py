...
class Graph:
    ...
    def bfs(self, starting_vertex, destination_vertex):
        # self.vertices = {0: {'n': 1, 's': 5, 'e': 3, 'w': 7}, 1: {'n': 2, 's': 0}, 2: {'s': 1}, 3: {'e': 4, 'w': 0}, 4: {'w': 3}, 5: {'n': 0, 's': '?'}, 6: {'n': '?', 'w': 11}, 7: {'e': 0, 'w': 8}, 8: {'e': 7}, 9: {'n': '?', 's': 10}, 10: {'n': 9, 'e': '?'}, 11
        # : {'e': 6, 'w': '?'}}
        ...
        while(q.size() > 0):
            ...
            if(last not in visited):
                ...
                for i in self.vertices[last]:
                    print(f"i: {i}")
                    if('n' == i):
                        deq.append(self.vertices[last]['n'])
                        c = deq.copy()
                        q.enqueue(c)
                        deq.pop()
                    if('s' == i):
                        deq.append(self.vertices[last]['s'])
                        c = deq.copy()
                        q.enqueue(c)
                        deq.pop()
                    if('e' == i):
                        deq.append(self.vertices[last]['e'])
                        c = deq.copy()
                        q.enqueue(c)
                        deq.pop()
                    if('w' == i):
                        deq.append(self.vertices[last]['w'])
                        c = deq.copy()
                        q.enqueue(c)
                        deq.pop()
                # COPY THE PATH
...
for i in roomGraph:
    newMap.add_vertex(i)
    currentMap.add_vertex(i)
    currentMap.vertices[i] = roomGraph[i][1]
    if("n" in roomGraph[i][1]):
        newMap.vertices[i]["n"] = "?"
    if("s" in roomGraph[i][1]):
        newMap.vertices[i]["s"] = "?"
    if("e" in roomGraph[i][1]):
        newMap.vertices[i]["e"] = "?"
    if("w" in roomGraph[i][1]):
        newMap.vertices[i]["w"] = "?"

print(newMap.vertices)
print(currentMap.vertices)

count = 0

while(count < len(roomGraph) + 2):

    random_room = random.randint(0, 3)
    direction = ["n", "s", "e", "w"]
    move = direction[random_room]
    print(count)
    # print(newMap.vertices)
    try:
        print("Trying")
        ...
        elif("?" not in newMap.vertices[player.currentRoom.id]):
            nearest_unexplored = newMap.bfs(player.currentRoom.id, "?")
            for i in nearest_unexplored:
                if(i != '?'):
                    if('n' in newMap.vertices[player.currentRoom.id]):
                        if(newMap.vertices[player.currentRoom.id]['n'] == i):
                            player.travel('n')
                            traversalPath.append('n')
                    if('s' in newMap.vertices[player.currentRoom.id]):
                        if(newMap.vertices[player.currentRoom.id]['s'] == i):
                            player.travel('s')
                            traversalPath.append('s')
                    if('e' in newMap.vertices[player.currentRoom.id]):
                        if(newMap.vertices[player.currentRoom.id]['e'] == i):
                            player.travel('e')
                            traversalPath.append('e')
                    if('w' in newMap.vertices[player.currentRoom.id]):
                        if(newMap.vertices[player.currentRoom.id]['w'] == i):
                            player.travel('w')
                            traversalPath.append('w')

            print(f'nearest unexplored in else: {nearest_unexplored}')
            print(f"Current Room After BFS: {player.currentRoom.id}")
    except:
        print(f"currentroom: {player.currentRoom.id}")
        print("Can't move in that direction")
        if(len(roomGraph) < 50):
            if(count == len(roomGraph)):
                break
